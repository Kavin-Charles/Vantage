...
describe('seedGroups', () => {
  it('returns the five seed groups with General as the only default', () => {
    ...
    expect(groups.find(g => g.label === 'Infra')!.item_keys).toEqual(['/infra/servers', '/infra/databases', '/infra/websites', '/infra/alerts'])
    expect(groups.find(g => g.label === 'Insights')!.item_keys).toEqual(['/analytics'])
