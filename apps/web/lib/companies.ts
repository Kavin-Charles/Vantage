...
export async function createCompany(token: string, body: Partial<Company>) {
  return apiFetch<{ data: Company; error: null }>('/api/companies', { method: 'POST', body: JSON.stringify(body), token });
}

export async function updateCompany(token: string, id: string, body: Partial<Company>) {
  return apiFetch<{ data: Company; error: null }>(`/api/companies/${id}`, { method: 'PATCH', body: JSON.stringify(body), token });
}
