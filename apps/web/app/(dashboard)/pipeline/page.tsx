...
export default function PipelinePage() {
  ...
  return (
    <>
      <Topbar
        left={<PipelineSwitcher value={pipelineId} onChange={setPipelineId} />}
        action={
          stages.length > 0 ? (
            <Button
              variant="primary"
      ...
      <div style={{ padding: 24 }}>
        ...
        {deals.length > 0 && (
          <div style={{ marginBottom: 20, fontSize: 13, color: 'var(--text2)' }}>
            {deals.length} deals ·{' '}
            <strong style={{ color: 'var(--text)' }}>{fmtValue(totalValue)}</strong> in pipeline
          </div>
        )}
      ...
      {modal && pipelineId && (
        <Modal
          title={modal === 'create' ? 'Add deal' : 'Edit deal'}
