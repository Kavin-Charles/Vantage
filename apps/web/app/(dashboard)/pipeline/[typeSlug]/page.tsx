...
interface Pipeline { id: string; name: string; record_type_id: string | null; }
    ...
    <>
      <Topbar
        left={
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <span style={{ fontFamily: 'Instrument Serif, serif', fontSize: 18, color: 'var(--text)' }}>{activeType.name}</span>
            {pipelines.length > 1 && (
              <select
                value={pipeline?.id ?? ''}
                onChange={e => setActivePipelineId(e.target.value)}
                style={{ border: '1px solid var(--border)', borderRadius: 6, padding: '4px 8px', fontSize: 13, fontFamily: 'DM Sans, sans-serif', background: 'var(--surface)', color: 'var(--text)' }}
              >
      ...
      <div style={{ padding: 24 }}>
        {pipeline ? (
          ...
          <p style={{ color: 'var(--text3)', fontSize: 14, fontFamily: 'DM Sans, sans-serif' }}>No pipeline found for this record type.</p>
        )}
      </div>
    </>
  );
