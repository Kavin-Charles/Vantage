...
function CriticalAlertsWidget({ config }: { config: WidgetConfig }) {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['widget', 'critical-alerts'],
    queryFn: async () =>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <WidgetHeader label="Critical Alerts" href="/alerts" />
      {alerts.length === 0 ? (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ fontSize: 13, color: 'var(--green)', fontWeight: 600 }}>✓ No critical alerts</span>
        </div>
      ) : (
...
registerDashboardWidget({
  id: 'insights:critical-alerts',
  label: 'Critical Alerts',
  description: 'Unresolved critical severity alerts',
  icon: 'warning',
  category: 'insights',
