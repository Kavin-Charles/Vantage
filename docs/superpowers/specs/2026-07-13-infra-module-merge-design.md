...
- **Gate generalization:** `createRequireCrmFeature` in `apps/api/src/middleware/module.ts` becomes `createRequireModuleFeature(db)` — curried: `requireModuleFeature(parentId)` returns a `(childId) => middleware` factory; the middleware passes only when BOTH parent and child modules are enabled. CRM call sites in `apps/api/src/index.ts` are refactored onto it. Infra mounts wired: `/api/servers` and `/api/servers/:id/ssh` → (`infra`, `infra:servers`); `/api/databases` → (`infra`, `infra:databases`); `/api/websites` → (`infra`, `infra:websites`); `/api/alerts` and `/api/alert-thresholds` → (`infra`, `infra:alerts`).
- **`/api/agent` heartbeat:** mounted without module middleware today (agents authenticate by `agent_token` inside the router) — stays ungated, unchanged. The `/agent` entry in `apiPrefixes` is registry metadata only.
