...
**Goal:** Add drag-and-drop and click-to-detail to the pipeline kanban board for deal-type pipelines.

**Architecture:** Extract board logic from inline `DealBoard` fn in `page.tsx` into a standalone `DealKanban` component (mirrors existing `RecordKanban`). Component owns its own drag state, drop mutation, and detail/create modals. `DealsList` delegates to it when `view === 'board'`.

**Tech Stack:** React, TanStack Query (`useMutation`, `useQueryClient`), HTML5 drag-and-drop API, `@vantage/api-client` `updateDeal`, `@vantage/types`.
...
|---|---|---|
| **Create** | `apps/web/components/pipeline/DealKanban.tsx` | Kanban board: columns, drag/drop, deal detail modal, create modal |
| **Modify** | `apps/web/app/(dashboard)/pipeline/[pipelineId]/page.tsx` | Remove `DealBoard` fn; swap in `<DealKanban>`; scope `createModal` to list view only |

---

### Task 1: Create `DealKanban` component

**Files:**
- Create: `apps/web/components/pipeline/DealKanban.tsx`
...
npx tsc --noEmit
```
Expected: no errors related to `DealKanban.tsx`.

- [ ] **Step 3: Commit**

```bash
git add apps/web/components/pipeline/DealKanban.tsx
git commit -m "feat: add DealKanban component with drag-and-drop and deal detail"
```

---

### Task 2: Wire `DealKanban` into the pipeline page

**Files:**
- Modify: `apps/web/app/(dashboard)/pipeline/[pipelineId]/page.tsx`

**What changes:**
1. Add import for `DealKanban`
2. Remove the inline `DealBoard` function (lines 76–126)
3. In `DealsList`: scope the `addTrigger` → `createModal` effect to list view only (`view !== 'board'`)
4. Scope the create `Modal` render to list view only
5. Replace the `view === 'board'` branch content with `<DealKanban pipelineId={pipelineId} addTrigger={addTrigger} />`

- [ ] **Step 1: Add DealKanban import**

In `apps/web/app/(dashboard)/pipeline/[pipelineId]/page.tsx`, add to the import block (after the `RecordKanban` import line):

```ts
import { DealKanban } from '@/components/pipeline/DealKanban';
```

- [ ] **Step 2: Remove inline `DealBoard` function**

Delete the entire `DealBoard` function (the block starting with `// ── Deal Board (kanban) ───` through the closing `}` of the function, approximately lines 74–126).

- [ ] **Step 3: Scope `addTrigger` effect to list view**
...
- [ ] **Step 4: Replace board branch content and scope create modal**

Change the board render branch from:
```tsx
) : view === 'board' ? (
  <DealBoard
    deals={activeDeals}
...
1. Open the pipeline page in the browser
2. Switch to Board view using the toggle
3. Confirm deal cards appear grouped by stage
4. Drag a card to a different column — confirm the deal moves and UI updates
5. Click a card — confirm detail modal opens with deal name, value, stage, close date
6. Click Edit in the modal — confirm the DealForm opens
7. Click "+ Add item" — confirm the create modal opens
8. Switch back to List view — confirm list renders correctly and "+ Add item" still works

- [ ] **Step 7: Commit**

```bash
git add apps/web/app/(dashboard)/pipeline/\[pipelineId\]/page.tsx
git commit -m "feat: wire DealKanban into pipeline page, replace static DealBoard"
```
